...
public class UrednikBean implements Serializable{
    ...
    public String ulogujSe() {
        ...
        try {
            if (!korisnickoIme.equals("") && !korisnickaSifra.equals("")) {
                urednikDB = logika.Kontroler.getInstance().ulogujUrednika(new Urednik(0, null, null, korisnickoIme, korisnickaSifra));
            }
        } catch (Exception ex) {
    ...
    public String logout() {
        urednikDB = null;
        FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("urednik");
        return "login";
    }
}
